...
def arc_length(theta):
    # arc length from theta[0] to theta[-1]
    integrand = np.sqrt((r0 + delta * theta / 2 / pi) ** 2 + (delta / 2 / pi) ** 2)
    s = integrate.cumtrapz(integrand, theta, initial=0)
    if theta[0] == 0:
        s0 = 0
    else:
        # arc length from theta=0 to theta[0]
        theta0 = np.linspace(0, theta[0], int(theta[0] / 2 / pi) * 60)
        integrand = np.sqrt((r0 + delta * theta0 / 2 / pi) ** 2 + (delta / 2 / pi) ** 2)
        s0 = integrate.trapz(integrand, theta0)
    return s + s0
...
f1_interp = interp.interp1d(s_data, f1_data)


def f1_comsol(theta):
    return f1_interp(arc_length(theta))
...
s_data = comsol[:, 0]
f2_data = comsol[:, 1]
f2_interp = interp.interp1d(s_data, f2_data)


def f2_comsol(theta):
    return f2_interp(arc_length(theta))
...
s_data = comsol[:, 0]
g1_data = comsol[:, 1] / mu
g1_interp = interp.interp1d(s_data, g1_data)


def g1_comsol(theta):
    return g1_interp(arc_length(theta))
...
s_data = comsol[:, 0]
g2_data = comsol[:, 1]
g2_interp = interp.interp1d(s_data, g2_data)


def g2_comsol(theta):
    return g2_interp(arc_length(theta))
