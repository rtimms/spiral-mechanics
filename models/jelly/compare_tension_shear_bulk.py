...
paths = ["Ks1_mus1/", "Ks1_mus1e-1/", "Ks1_mus1e-2/", "Ks1e-1_mus1e-1/", "Ks1e-1_mus1e-2/", "Ks1e-2_mus1e-2/"]  # paths to data
...
for K_s, mu_s, path, linestyle in zip(K_s_list, mu_s_list, paths, linestyles):
    ...
    comsol = ComsolSolution(
        r0, delta, l_n, l_s, l_p, hh, N, lam_n, lam_s, lam_p, mu_n, mu_s, mu_p, alpha_scale, "data/jelly/"+path
    )
    ...
    ax[1].plot(
        ...
        comsol["Tp"], 
        linestyle, 
        label=r"$K_s/K_a=$"+f"{K_s}, " +  r"$\mu_s/\mu_a=$"+f"{mu_s}",   
    )
...
plt.tight_layout()    
plt.savefig("figs/jelly/compare_mu_s/T_of_theta.pdf", dpi=300)
