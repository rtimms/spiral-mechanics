...
paths = ["data/jelly/N5/", "data/jelly/3D-N5/"]  # path to data
labels = ["2D", "3D"]
for path, label in zip(paths, labels):
    ...
    ax[0].plot(theta, comsol.Tp, "--", label=label)
    ax[1].plot(theta, comsol.Tn, "--", label=label)
    ax[2].plot(theta, comsol.Tn + comsol.Tp, "--", label=label)
...
plt.tight_layout()
plt.savefig("figs/compare-tension/compare-tension.pdf", dpi=300)
plt.show()
